...
public class XAPKZipResourceFile {
    ...
    public XAPKZipResourceFile(String zipFileName) throws IOException {
    ...
    ZipEntryRO[] getEntriesAt(String path) {
        ...
        for (ZipEntryRO ze : values) {
            if (!ze.mFileName.startsWith(path))
                continue;
            if (ze.mFileName.indexOf('/', length) != -1)
                continue;
            zev.add(ze);
        }
    ...
    public void addPatchFile(String zipFileName) throws IOException {
        ...
        if (eocdIdx < 0)
            Log.d(LOG_TAG, "Zip: EOCD not found, " + zipFileName + " is not a zip file.");
