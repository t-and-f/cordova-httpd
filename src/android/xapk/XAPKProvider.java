...
public class XAPKProvider extends ContentProvider {
    ...
    public static final String[] ALL_FIELDS = { FILEID, FILENAME, ZIPFILE, MODIFICATION, CRC32, COMPRESSEDLEN,
            UNCOMPRESSEDLEN, COMPRESSIONTYPE };
    ...
    public static final int[] ALL_FIELDS_INT = { FILEID_IDX, FILENAME_IDX, ZIPFILE_IDX, MOD_IDX, CRC_IDX, COMPLEN_IDX,
            UNCOMPLEN_IDX, COMPTYPE_IDX };
    ...
    public AssetFileDescriptor openAssetFile(Uri uri, String mode) throws FileNotFoundException {
        if (initIfNecessary() == false)
            throw new FileNotFoundException();
        // Use uri.getPath() instead of uri.getEncodedPath(), because the paths inside
        // the ZIP
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        if (projection == null) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (projection[i].equals(FILENAME)) {
                    intProjection[i] = FILENAME_IDX;
                    continue;
                }
                if (projection[i].equals(ZIPFILE)) {
                    intProjection[i] = ZIPFILE_IDX;
                    continue;
                }
                if (projection[i].equals(MODIFICATION)) {
                    intProjection[i] = MOD_IDX;
                    continue;
                }
                if (projection[i].equals(CRC32)) {
                    intProjection[i] = CRC_IDX;
                    continue;
                }
                if (projection[i].equals(COMPRESSEDLEN)) {
                    intProjection[i] = COMPLEN_IDX;
                    continue;
                }
                if (projection[i].equals(UNCOMPRESSEDLEN)) {
                    intProjection[i] = UNCOMPLEN_IDX;
                    continue;
                }
                if (projection[i].equals(COMPRESSIONTYPE)) {
